...
const FulfillmentBarChart = ({ deliveries = [] }) => {
  const isDark = useDarkMode();
  
  const booksReadCount = deliveries.filter(d => 
    d.deliveryStatus === 'delivered'
  ).length;

  const pendingRequestsCount = deliveries.filter(d => 
    d.deliveryStatus === 'pending' || d.deliveryStatus === 'processing'
  ).length;

  const arrivingSoonCount = deliveries.filter(d => 
    d.deliveryStatus === 'dispatched' || d.deliveryStatus === 'shipped'
  ).length;

  const totalSum = booksReadCount + pendingRequestsCount + arrivingSoonCount;
    ...
    <div className="bg-white dark:bg-[#2c2f38] border border-slate-200/80 dark:border-gray-800 rounded-3xl p-6 md:p-8 flex flex-col justify-between h-full transition-colors duration-300">
      ...
      <div className="flex justify-between items-start mb-6">
        <div className="space-y-1">
          <h3 className="text-lg font-extrabold text-[#192230] dark:text-white">Library Delivery Log</h3>
          <p className="text-xs text-slate-500 dark:text-slate-400">Distribution of catalog actions requested and received</p>
        </div>
        <div className="flex flex-col items-end">
          <span className="text-2xl font-black text-[#856a26] dark:text-[#ffcd00]">{totalSum}</span>
          <span className="text-[10px] font-bold text-slate-400 uppercase">Items Total</span>
        </div>
      </div>
...
export default FulfillmentBarChart;