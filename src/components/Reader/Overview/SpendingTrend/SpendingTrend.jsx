...
// Set up past 7 days based on today's date
  ...
  deliveries.forEach(delivery => {
    if (delivery.createdAt) {
      ...
      if (match) {
        match.spent += parseFloat(delivery.paid || 0);
      }
    }
  });

  const weeklySpentTotal = last7Days.reduce((acc, curr) => acc + curr.spent, 0);
    ...
    <div className="bg-white dark:bg-[#2c2f38] border border-slate-200/80 dark:border-gray-800 rounded-3xl p-6 md:p-8 flex flex-col justify-between h-full transition-colors duration-300">
      ...
      <div className="flex justify-between items-start mb-6">
        <div className="space-y-1">
          <h3 className="text-lg font-extrabold text-[#192230] dark:text-white">Weekly Spending Trend</h3>
          <p className="text-xs text-slate-500 dark:text-slate-400">Activity flow of total delivery fees paid inside the last 7 days</p>
        </div>
        <div className="flex flex-col items-end">
          <span className="text-2xl font-black text-[#856a26] dark:text-[#ffcd00]">
            {formatCurrency(weeklySpentTotal)}
          </span>
          <span className="text-[10px] font-bold text-slate-400 uppercase">Paid This Week</span>
        </div>
      </div>
