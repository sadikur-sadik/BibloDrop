...
const EditBooks = ({ book, onEdit }) => {
  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      ...
      if (!apiKey) {
        throw new Error("ImgBB API Key is missing. Define NEXT_PUBLIC_IMGBB_KEY in your env file.");
      }
  ...
  const handleFormSubmit = async (e) => {
    ...
    const updatedData = {
      ...formDataObj,
      quantity: formDataObj?.quantity ? Number(formDataObj.quantity) : 0,
      deliveryFee: formDataObj?.deliveryFee ? Number(formDataObj.deliveryFee) : 0,
      genre,
      coverImage: coverUrl,
    };
  ...
  const handleGenreChange = (key) => {
    ...
    const rawGenre = keyString.startsWith(instanceId)
      ? keyString.replace(`${instanceId}-`, '')
      : keyString;
    setGenre(rawGenre);
  };
  ...
  return (
    <>
      {/* Redesigned Compact Trigger Button */}
      <motion.button
                ...
                <Form onSubmit={handleFormSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                  ...
                  <div className="space-y-4">
                    ...
                    <TextField isRequired name="description" defaultValue={book?.description} className="flex flex-col gap-1">
                      ...
                      <TextArea placeholder="Provide a summary of the book plot..." rows={6} className="rounded-xl" />
                      <FieldError className="text-xs text-rose-500 mt-1" />
                    </TextField>
                  </div>
                  ...
                  <div className="flex flex-col gap-4 justify-between">
                    <div className="space-y-4">
                      <div>
                        ...
                        {!coverUrl ? (
                          ...
                          <div className="relative border border-slate-200 dark:border-slate-800 p-3 rounded-2xl flex items-center gap-3 bg-slate-50 dark:bg-[#12141c]/20">
                            <img src={coverUrl} alt="Book Cover Preview" className="h-20 w-14 object-cover rounded-lg shadow-sm border dark:border-slate-800" />
                            <div className="flex flex-col min-w-0 flex-1">
                              <span className="text-xs font-bold text-slate-700 dark:text-slate-200 truncate">Cover image loaded</span>
                              <a href={coverUrl} target="_blank" rel="noreferrer" className="text-[10px] text-amber-700 dark:text-[#ffcd00] hover:underline mt-0.5 truncate">
                                View Full Image
                              </a>
                            </div>
                            <Button isIconOnly type="button" variant="light" onClick={resetCoverImage} className="text-slate-400 hover:text-rose-500 rounded-full cursor-pointer">
                          ...
                          {isUploading && (
                            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden mt-3">
                              ...
                              <div className="w-full h-1.5 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden border dark:border-slate-800">
                                <motion.div className="h-full bg-[#856a26] dark:bg-[#ffcd00] rounded-full" initial={{ width: 0 }} animate={{ width: `${uploadProgress}%` }} transition={{ ease: "easeOut" }} />
                              </div>
                            </motion.div>
                          )}
                        </AnimatePresence>
                      ...
                      <Select 
                        ...
                        selectedKey={genre ? `${instanceId}-${genre}` : ""}
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        ...
                        <TextField
                          ...
                          name="deliveryFee"
                          type="number"
                          defaultValue={book?.deliveryFee?.toString() || "0"}
                          className="flex flex-col gap-1"
                          validate={(value) => {
                        ...
                        >
                          ...
                          <Input min="0" step="0.01" placeholder="0.00" className="rounded-xl" />
                          <FieldError className="text-xs text-rose-500 mt-1" />
                        </TextField>
                      </div>
                    </div>
                    ...
                    <div className="flex gap-2.5 justify-end pt-4 border-t border-slate-100 dark:border-gray-800/60">
                      ...
                      <Button type="submit" isLoading={isSubmitting} disabled={isUploading} className="px-5 h-10 rounded-xl text-xs font-bold bg-[#856a26] dark:bg-[#ffcd00] text-white dark:text-[#192230] hover:bg-slate-800 dark:hover:bg-white transition-colors cursor-pointer">
