...
const PublishToggle = ({ 
  ...
  // --- Variant 1: Text Button (Perfect height and text scale for inventory grids) ---
  if (variant === 'button') {
    return (
      <motion.button
        ...
        className={`w-full h-9 px-3 rounded-xl text-[11px] font-bold tracking-tight flex items-center justify-center gap-1.5 transition-all cursor-pointer ${
          isPending
      ...
      >
        {isPending ? (
          ...
          <>
            <EyeSlash className="w-3.5 h-3.5" /> Unpublished
          </>
        )}
      </motion.button>
    );
  ...
  // --- Variant 2: Icon-only Tooltip (Sized for table rows) ---
  return (
    <div className="relative group flex items-center justify-center">
      <motion.button
        ...
        onClick={handlePress}
        disabled={isPending || isToggling}
        className={`h-9 w-9 rounded-xl transition-colors cursor-pointer flex items-center justify-center ${
          isPending
